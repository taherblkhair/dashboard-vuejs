...
const exportCsv = () => {
  ...
  const headers = ['id', 'code', 'supplier', 'status', 'total_amount', 'order_date', 'expected_delivery_date']
  const rows = orders.value.map(o => [
    ...
    escapeCsv(o.expected_delivery_date)
  ])
  ...
  a.href = url
  a.download = `purchase-orders-page-${meta.value.current_page}.csv`
...
const exportExcel = () => {
  if (!orders.value.length) return alert('لا توجد بيانات للتصدير')
  const headers = ['الرمز', 'المورد', 'الحالة', 'الإجمالي', 'تاريخ الطلب', 'تاريخ التوصيل المتوقع']
  const rows = orders.value.map(o => [o.code, o.supplier?.name || '', o.status, o.total_amount, formatDate(o.order_date), formatDate(o.expected_delivery_date)])
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `purchase-orders-page-${meta.value.current_page}.xls`
  document.body.appendChild(a)
  a.click()
  a.remove()
  URL.revokeObjectURL(url)
