      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between py-4 gap-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <button v-if="['confirmed', 'processing'].includes(order?.status) && !delivery" @click="openCreateDelivery" class="inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all">
...
const orderTimelineSteps = computed(() => {
  ...
  if (!delivery.value) return steps.filter(s => s.key !== 'shipped')
  return steps
})
const currentOrderStepIndex = computed(() => orderTimelineSteps.value.findIndex(step => step.key === (order.value?.status || '').toLowerCase()))
...
const orderProgressPercentage = computed(() => {
  ...
  return (currentOrderStepIndex.value / (orderTimelineSteps.value.length - 1)) * 100
})
