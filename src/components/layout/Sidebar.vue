<template>
  <div>
    ...
    >
          ...
          <div v-if="!item.children" class="menu-item">
            ...
            >
              <div class="relative flex-shrink-0">
                <div
                  class="w-10 h-10 rounded-lg bg-slate-700/50 flex items-center justify-center group-hover:bg-blue-500/20 transition-colors"
                >
                  <svg
                    class="w-5 h-5"
                    fill="none"
                ...
                <span
                  v-if="item.badge"
                  class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center border-2 border-slate-800"
                >
                  {{ item.badge }}
                </span>
              </div>
              ...
              >
                ...
                >
                  <div class="relative">
                    ...
                    <span
                      v-if="child.badge"
                      class="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border border-slate-800"
                    ></span>
                  </div>
      ...
      <div
        v-show="!isCollapsed || hovered"
        class="p-4 border-t border-slate-700/50"
      >
...
const asideClasses = computed(() =>
  [
    "fixed lg:sticky inset-y-0 left-0 z-50 bg-slate-900/90 backdrop-blur-xl text-white transform transition-all duration-300 ease-in-out flex flex-col border-r border-white/10 shadow-2xl",
    props.isOpen ? "translate-x-0" : "-translate-x-full",
    "lg:translate-x-0 lg:h-screen lg:top-0",
    isCollapsed.value ? "lg:w-20" : "lg:w-72",
    hovered.value && isCollapsed.value ? "lg:w-72 shadow-blue-500/10" : "",
  ].join(" ")
);
...
.menu-item::before {
  content: "";
  @apply absolute -left-1 top-1/2 transform -translate-y-1/2 w-1.5 h-0 bg-blue-500 rounded-full transition-all duration-300 shadow-[0_0_8px_rgba(59,130,246,0.5)];
}
...
.submenu-item::before {
  content: "";
  @apply absolute left-0 top-1/2 -translate-y-1/2 w-1 h-0 bg-blue-500/50 rounded-full transform -translate-x-full transition-all duration-300;
}

.submenu-item:hover::before,
.submenu-item.router-link-active::before {
  @apply h-1/2 translate-x-0;
}

/* User menu item */
.user-menu-item {
  @apply flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm hover:bg-white/10 transition-all duration-200;
}

/* Quick action button */
.quick-action-btn {
  @apply flex flex-col items-center justify-center gap-1.5 p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all duration-200;
}
